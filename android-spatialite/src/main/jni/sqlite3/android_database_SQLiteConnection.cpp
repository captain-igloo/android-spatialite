...
struct SQLiteConnection {
    ...
    SQLiteConnection(sqlite3* db, int openFlags, const char* path_, const char* label_, const void* spatialiteCache) :
        db(db), openFlags(openFlags), canceled(false), spatialiteCache(spatialiteCache) {
        path = strdup(path_);
        label = strdup(label_);
    }
...
static jlong nativeOpen(JNIEnv* env, jclass clazz, jstring pathStr, jint openFlags,
    ...
    SQLiteConnection* connection = new SQLiteConnection(db, openFlags, pathChars, labelChars, spatialiteCache);
